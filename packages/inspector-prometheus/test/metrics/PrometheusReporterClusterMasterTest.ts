...
export class PrometheusReporterClusterMasterTest {
    ...
    @test
    public async "check if unexpected response messages are ignored"() {
    ...
    @test
    public async "check if master process is trying to send messages to workers"() {
    ...
    @test
    public async "check if timeout for worker responses is working"() {
        const worker = {};
    ...
    @test
    public async "check if request message contains all required fields"() {
        const worker = {};
        this.clusterOptions.workers.push(worker as any);
