...
export class BaseMetricTest {
  ...
  public 'set multiple values with fluent interface'(): void {
    ...
    baseMetric.setMetadata('config1', 456).setMetadata('config2', 'config test 2').setTag('application', 'metric-app2')
    ...
    baseMetric.removeTag('application').removeMetadata('config2')
  ...
  public 'check getMetricDescription'(): void {
    const serializedMetric = JSON.parse(
      JSON.stringify(new TestMetric().setName('Test1').setDescription('description 1'))
    )
    expect(getMetricDescription(serializedMetric)).to.equal('description 1')
    expect(getMetricDescription(new TestMetric().setName('Test2').setDescription('description 2'))).to.equal(
      'description 2'
    )
  }
  ...
  public 'check getMetricGroup'(): void {
    const serializedMetric = JSON.parse(JSON.stringify(new TestMetric().setName('Test1').setGroup('group 1')))
    expect(getMetricGroup(serializedMetric)).to.equal('group 1')
    expect(getMetricGroup(new TestMetric().setName('Test2').setGroup('group 2'))).to.equal('group 2')
  }

  @test
  public 'check getMetricTags'(): void {
    const serializedMetric = JSON.parse(
      JSON.stringify(new TestMetric().setName('Test1').setTag('key1', 'value1').setTag('key2', 'value2'))
    )
    ...
    const metric = new TestMetric().setName('Test1').setTag('key1', 'value1').setTag('key2', 'value2')
  ...
  public 'check getMetricMetadata'(): void {
    const serializedMetric = JSON.parse(
      JSON.stringify(new TestMetric().setName('Test1').setMetadata('key1', 1).setMetadata('key2', 'value2'))
    )
    ...
    const metric = new TestMetric().setName('Test1').setMetadata('key1', 3).setMetadata('key2', 'value4')
